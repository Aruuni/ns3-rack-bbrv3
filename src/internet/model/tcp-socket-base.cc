  ...
  if (m_txBuffer->GetLost() != 0 && m_tcb->m_congState == TcpSocketState::CA_DISORDER)
    {
        ...
        m_congestionControl->MarkedLoss(m_tcb, rateConn, m_txBuffer);
        EnterRecovery(m_txBuffer->GetSacked()); // ??
        NS_ASSERT (m_tcb->m_congState == TcpSocketState::CA_RECOVERY);
    }
...
TcpSocketBase::DupAck(uint32_t currentDelivered)
{
    ...
    else if (m_tcb->m_congState == TcpSocketState::CA_DISORDER)
    {
        ...
        //     TCP socket buffer size at receiver side.
         ...
         ((m_dupAckCount == m_retxThresh) || (!m_recoverActive)))
...
TcpSocketBase::ReceivedAck(Ptr<Packet> packet, const TcpHeader& tcpHeader)
{
    ...
    if (m_rackEnabled && ackNumber >= oldHeadSequence)
    {
        ...
        if (!(bytesSacked > 0))
        {
            ...
            m_rack->UpdateStats (tser, item.m_retrans, item.GetLastSent(), ackNumber, m_tcb->m_nextTxSequence, rtt);
        }
        ...
        else 
        {
            ...
            m_rack->UpdateStats (tser, item.m_retrans, item.GetLastSent(), highestSacked, m_tcb->m_nextTxSequence, rtt);
        }
