...
void 
TcpBbr::bbr_handle_inflight_too_high(Ptr<TcpSocketState> tcb, const TcpRateOps::TcpRateSample& rs, bool rsmode)
{
...
TcpBbr::bbr_probe_inflight_hi_upward(Ptr<TcpSocketState> tcb, const TcpRateOps::TcpRateSample& rs)
{
    ...
    if (m_bwProbeUpAcks >= m_bwProbeUpCount)
    {
        ...
        //std::cout <<  "probe up acks " << m_bwProbeUpAcks << " m_bwProbeUpCount: " << m_bwProbeUpCount << std::endl;
        uint32_t delta = m_bwProbeUpAcks / m_bwProbeUpCount;
        m_bwProbeUpAcks -= delta * m_bwProbeUpCount;
        COUT("inflight hi " << m_inflightHi << " delta " << delta << " at time" << NOW);
...
{
    NS_LOG_FUNCTION(this << tcb);   
    //std::cout << "m_bwProbeUpRounds before: " << m_bwProbeUpRounds << std::endl;      m_initialCWnd
    uint32_t growth_this_round = 1 << m_bwProbeUpRounds;
    m_bwProbeUpRounds = std::min<uint32_t>(m_bwProbeUpRounds + 1, 30);
    //std::cout << "growth_this_round: " << growth_this_round << std::endl;
        ...
        case BBR_BW_PROBE_UP:
            ...
            if (m_prevProbeTooHigh && inflight >= m_inflightHi) {
                ...
                if (inflight >= bbr_inflight(tcb, bw, m_pacingGain))   {       //bbr_inflight(tcb, bw, m_pacingGain)) {
                    ...
                    is_bw_probe_done = true;
                    std::cout << "stopped by bw now" << std::endl;
                }
            } 
            if (is_bw_probe_done)
            {
                COUT ("BBR_BW_PROBE_ at time " << NOW);
...
void 
TcpBbr::bbr_inflight_hi_from_lost_skb(Ptr<TcpSocketState> tcb, const TcpRateOps::TcpRateSample& rs)
{
...
void 
TcpBbr::bbr_note_loss(Ptr<TcpSocketState> tcb, uint32_t delivered)
{
...
void
TcpBbr::MarkedLoss(Ptr<TcpSocketState> tcb, const TcpRateOps::TcpRateConnection& rc, Ptr<TcpTxBuffer> buff)
{
    NS_LOG_FUNCTION(this << tcb);
    bbr_note_loss(tcb, rc.m_delivered);

    //std::cout << "Marked Loss at time " << Now().GetSeconds() <<  " buff >> " << buff->BytesInFlight() << std::endl;
    if (!m_bwProbeSamples)
        return;
    // if (!buff->IsRetransmittedDataAcked())
    //     return;
    auto m_rateOps = CreateObject<TcpRateLinux>();
    
    auto rs = m_rateOps->GenerateSample(0, buff->GetLost(), false, buff->BytesInFlight() , tcb->m_minRtt);

    if (bbr_is_inflight_too_high(tcb, rs)){
        rs.m_priorInFlight = m_inflightHi;
